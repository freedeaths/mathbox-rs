...
use crate::opt::utils::{local_max, argsort};
...
pub fn dft_filter_lowpass(signal: &[f64], dt: f64, cutoff_frequency: f64, top_n: usize) -> (Vec<f64>, Vec<f64>) {
    ...
    let prominence = (f_amp.iter().copied().fold(f64::NAN, f64::max) - f_amp.iter().copied().fold(f64::NAN, f64::min)) * 0.001;
    ...
    let top_n_amp_pos = top_amp_pos.iter().map(|x| amp_max_pos[*x]).collect::<Vec<_>>();
    // cut small amplitude
    for i in 0..f_sig.len() {
        if !top_n_amp_pos.contains(&i) {
            f_sig[i] = Complex::new(0., 0.);
        }
    }
...
mod tests {
    ...
    fn test_dft_filter_lowpass() {
        ...
        let (_, result) = dft_filter_lowpass(&signal, 1.0, 0.0, 10000);
        let expected = vec![1.177546665236456, 2.220297731272131, 3.4049979717345273, 3.708219789800072, 2.8680187999032034, 1.610219856779601, 1.021398370548015, 1.6102198567796013, 2.8680187999032127, 3.708219789800072, 3.4049979717345247, 2.2202977312721237, 1.1775466652364555];
        for i in 0..signal.len() {
            assert_relative_eq!(result[i], expected[i]);
        }
    }
